...
# from files_db_hidden import hide_file as notthisfile
# from files_db_hidden import hide_folder as notthisfolder
...
def main():
    ...
    if folder_item_nr in ('y', 't'):
        ...
        for k in lista_folderow_count:
            if any(substring in k[1].replace('\\', ' ') for substring in notthisfolder()):
                continue
                ...
                for m in filelist(k[1]):
                    ...
                    if extension.lstrip('.') in notthisfile():
                        ...
                        (name, k[0], stctime(k[1]+'\\'+m), extension.lstrip('.')))
